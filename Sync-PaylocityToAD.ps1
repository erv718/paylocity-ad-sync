            ...
            1 {
                ...
                if ($WillChangeAD) {
                    ...
                    Move-ADObject -Identity $u.DistinguishedName `
                                  -TargetPath "OU=Users,OU=Disabled,OU=[Company],DC=SANDBOX,DC=local"
...
if ($newAccounts.Count) {
    ...
    foreach ($r in $newAccounts) {
        ...
        $ln = $r.'Last Name'
        # build & sanitize base SamAccountName
        $baseSam = ("{0}.{1}" -f $fn,$ln).ToLower() -replace '[^a-z0-9\.]',''
        if ($baseSam.Length -gt 20) { $baseSam = $baseSam.Substring(0,20) }

        # collision resolution: append 01,02,...
        $sam = $baseSam
        $suffix = 1
        while (Get-ADUser -Filter "SamAccountName -eq '$sam'" -ErrorAction SilentlyContinue) {
            $sam = $baseSam + ('{0:00}' -f $suffix)
            $suffix++
            if ($suffix -gt 99) { break }
        }
        ...
        if ($WillChangeAD) {
            ...
            Write-Host "   [Simulation] Would create $sam with password '$pw'."
        }
    }
} else {
